...
#define LUMBERJACK_RENT_POS -924.5295, -536.3654, 26.1405
#define LUMBERJACK_RENT_POS_CAR -931.1045, -537.2745, 25.6807
...
CMD:edittime(playerid, params[])
{
	extract params -> new i; else return SendClientMessage(playerid, -1, "{FFFF00}SYNTAX: {FFFFFF}/edittime [treeid]");
	...
	g_tree[i][T_TIME] = 0;
	TreeObjectInit(i, OBJECT_INIT_TYPE_UPDATE);
 	SendClientMessage(playerid, -1, "{FF0000}info: {FFFFFF}perfect ok");
	return 1;
}
...
CMD:ctree(playerid, params[])
{
	...
	g_player_tree[playerid][P_EDITING_TYPE] = EDITING_TYPE_CREATE;
	g_player_tree[playerid][P_EDITING_OBJECT] = CreateDynamicObject(618, x + 2.0, y + 2.0, z - 1, 0, 0, 0);
    CreateTrees(x, y, z - 1, 0, 0, 0);
	DestroyDynamicObject(g_player_tree[playerid][P_EDITING_OBJECT]);
...
CMD:unloadlumber(playerid, params[])
{
    ...
    switch(WOOD_STOCK)
    {
        case 0..300: price = 1500;
        case 301..800: price = 1500;
        case 801..900: price = 1500;
        default: price = 1500;
    }
...
CMD:loadlumber(playerid, params[])
{
	...
	if(!g_player_tree[playerid][P_HAS_WOOD])
