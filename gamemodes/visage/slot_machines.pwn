...
hook OnScriptInit( )
{
	...
	mysql_function_query( connectionID, "SELECT * FROM `CASINO_POOLS`", true, "OnCasinoPoolsLoad", "" );
...
thread OnCasinoPoolsLoad( )
{
    ...
    if ( rows )
    {
    	...
    	{
	    	new
	    		poolid = cache_get_field_content_int( i, "ID", connectionID );
	    	...
	    	if ( 0 <= poolid < MAX_SLOT_POOLS )
	    	{
	    		...
	    		// insert data
	    		g_casinoPoolData[ poolid ] [ E_POOL ] 			= cache_get_field_content_int( i, "POOL", connectionID );
	    		g_casinoPoolData[ poolid ] [ E_TOTAL_WINNINGS ] = cache_get_field_content_int( i, "TOTAL_WINNINGS", connectionID );
	    		g_casinoPoolData[ poolid ] [ E_TOTAL_GAMBLED ] 	= cache_get_field_content_int( i, "TOTAL_GAMBLED", connectionID );
		...
		mysql_function_query( connectionID, "SELECT * FROM `SLOT_MACHINES`", true, "OnSlotMachinesLoad", "" );
    }
    return 1;
}
...
thread OnSlotMachinesLoad( )
{
    ...
    if ( rows )
    {
    	for( new i = 0; i < rows; i++ )
    	{
			...
			{
				new
					Float: X = cache_get_field_content_float( i, "X", connectionID ),
					Float: Y = cache_get_field_content_float( i, "Y", connectionID ),
					Float: Z = cache_get_field_content_float( i, "Z", connectionID ),
					Float: rZ = cache_get_field_content_float( i, "ROTATION", connectionID ),
					Float: fOffsetX,
					Float: fOffsetY
				;
				...
				// Load variables
				g_slotmachineData[ id ] [ E_ENTRY_FEE ] = cache_get_field_content_int( i, "ENTRY_FEE", connectionID );
				g_slotmachineData[ id ] [ E_POOL_ID ] = cache_get_field_content_int( i, "POOL_ID", connectionID );
